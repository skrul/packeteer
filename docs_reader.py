...
SAMPLE_DOCS = [
    ...
    "https://docs.google.com/document/d/18cQuERD6Fh0Ppcwhpdjp3-CCV_6rg84vWqUHCQeA3uc/edit?tab=t.0#heading=h.prg65i6ip6mcere ",
...
def main():
    ...
    for i, url in enumerate(SAMPLE_DOCS, 1):
        ...
        if html_content:
            ...
            links = re.findall(
                r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', html_content, re.IGNORECASE
            )
            if links:
                print("Found links:")
